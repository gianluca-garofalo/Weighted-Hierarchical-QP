...
class Task0 : public hqp::TaskInterface<> {
  private:
    void run() override {
        matrix_ = (Eigen::MatrixXd(6, 3) << Eigen::MatrixXd::Identity(3, 3), -Eigen::MatrixXd::Identity(3, 3)).finished();
...
int main() {
    ...
    LexLS::internal::LexLSI lexls(
        A.cols(), break_points.size(), &number_of_constraints[0], &types_of_objectives[0]);
    ...
    return daqp.get_primal().isApprox(lexls.get_x(), precision) &&
           hqp.get_primal().isApprox(lexls.get_x(), precision)
           ? 0
           : 1;
}
